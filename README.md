...
ต้องครบทุกข้อ (จูนจาก 5000-bar sweep ให้ win rate สูงสุดที่ยังมีจำนวนเทรดพอเชื่อถือได้):
setup score ≥ 50 · อยู่ในเวลาเทรด (London+NY) · ADX ≥ 18 · **MA 9/21/50/200 เรียงตัว ≥ 3/4** ·
ราคาอยู่ที่ระดับสำคัญ (โซน/POC/EMA/BOS) · แท่ง rejection ปิดยืนยัน · MACD histogram หันทางเทรด ·
**RSI ยืนยันฝั่ง** (BUY ต้อง >55, SELL ต้อง <45 — ไม่ใช่แค่ไม่สุดขั้ว) · SELL ต้องหลุด EMA200 ก่อน
`entrySignal.fire` = true → เข้าได้ · ไม่ครบ → `entrySignal.blockers` บอกว่าขาดอะไร

MA-เรียงตัว + RSI-ยืนยัน คือ 2 ตัวกรองที่ยกเปอร์เซ็นต์ชนะได้มากสุดในการ sweep — ทำให้ trigger
เข้มขึ้น (เทรดน้อยลง) แลกกับความแม่นที่สูงขึ้นชัดเจน (ดูตารางด้านล่าง)
...
|---|---|---|---|---|---|---|---|
| **H4** ✅ | ~2.8 ปี | 25 | **65%** | **+0.62R** | +15.5R | **2.9** | **~4R** |
| **M30** ✅ (sample เล็ก) | ~3.5 เดือน | 13 | 55% | +0.32R | +4.2R | 1.8 | ~5R |
| H1 ❌ | ~7 เดือน | 16 | 21% | −0.54R | −8.7R | 0.31 | 9.9R |
| M15 ❌ | ~1.7 เดือน | 13 | 13% | −0.39R | −5.1R | 0.43 | 6R |

M30 ใช้เกณฑ์เดียวกับ H4 **บวก** "โครงสร้าง TF ถัดไป (H1) ต้องเห็นด้วยกับทิศทาง" (`requireStructAgree`) —
ไม่มีตัวกรองไหนกู้ H1/M15 ได้ (ลองแล้ว >10 คอนฟิกต่อ TF รวมตัวกรองเดียวกันนี้ด้วย ยังติดลบทุกแบบ)
ทั้ง `institutional.entrySignal.reliable` และ dashboard บอกสถานะนี้ตรง ๆ ต่อกรอบเวลา — เลือกได้ทั้ง 4
TF แต่ระบบจะไม่เรียก M15/H1 ว่า "แนะนำ" เพราะ backtest ยังไม่พบ edge จริง ๆ

### Mean-reversion (ทดลอง, ยังไม่แนะนำ)

ลองมุมตรงข้าม — เทรนด์ตามไม่เวิร์กบน TF เล็ก ลองเก็งกำไรสวนกลับ (fade) แทน: `strategy.meanReversion()`
เข้าเมื่อราคาแตะขอบ Bollinger Band (20, 2σ) พร้อม RSI สุดขั้ว (≤32 หรือ ≥68) + แท่ง rejection ยืนยัน
**เฉพาะตอนตลาดไซด์เวย์** (ADX ≤ 22 — ตรงข้ามกับตัวกรอง trend-following) เป้าคือกลับไปเส้นกลาง Band
(TP1) แล้วรันไปฝั่งตรงข้าม (TP2) · เทสต์ผ่าน `GET /api/backtest?mode=meanreversion` หรือ
`runMeanReversionBacktest()`

**ผล: ยังไม่พบ edge เป็นบวก** —
- เข้มตามที่ออกแบบ (ADX≤22): แทบไม่มีสัญญาณเลย (0–7 เทรดใน 1.7–6.9 เดือน) เพราะทองปี 2026 เป็นเทรนด์แรง
  แทบไม่มีช่วงไซด์เวย์ให้เทรด — sample เล็กเกินจะสรุปได้
- ผ่อน ADX ให้ได้ sample มากขึ้น (≤30–40): เทรดได้ 8–27 ครั้ง แต่ **ติดลบชัดเจนทุกกรณี**
  (win 16–43%, exp −0.3 ถึง −0.5R) — และยิ่งผ่อนยิ่งแย่ลง สอดคล้องกับที่คาด: สวนเทรนด์แรงย่อมโดนบด

สรุป: mean reversion ไม่ใช่คำตอบสำหรับทองช่วงนี้ เพราะตลาดเทรนด์ ไม่ใช่ไซด์เวย์ — เก็บโค้ด/เทสต์ไว้
ให้เรียกดูได้ (เผื่อเจอช่วงตลาดไซด์เวย์จริงในอนาคต) แต่ไม่ผูกเข้ากับ `entrySignal`/dashboard สด

**เพดานที่ทดสอบได้จริง (sweep หลายสิบคอนฟิก): ~65–67%** ด้วยตัวกรอง MA เรียงตัว ≥3/4 + RSI ยืนยันฝั่ง
บน H4 — เข้มกว่านี้ (score≥70, ADX≥25, rr≥1.6, ...) เทรดเหลือ 6–15 ครั้งใน 34 เดือน ซึ่งน้อยเกินจะเชื่อถือ
ตัวเลขได้ (win rate แกว่ง 0–47% เพราะ sample เล็ก) **ไม่มีคอนฟิกไหนแตะ 80% แบบมี sample ที่เชื่อถือได้**
— mechanical gold system ที่กำไรจริงระยะยาวปกติอยู่ 45–65% ไม่ใช่ 80% · กำไรมาจาก **PF สูง**
(ไม้ชนะใหญ่กว่าไม้แพ้) ไม่ใช่ชนะบ่อย · H1 ลงมาไม่มี edge เลย ไม่ว่าจะกรองแค่ไหน ·
`runTriggerBacktest()` รับ `extraFilter(institutional, entrySignal) => bool` เพิ่มเติมได้
สำหรับทดลองเกณฑ์ของตัวเอง (ดูสคริปต์ทดลองในประวัติ commit)
