...
/**
 * Quasimodo (QM / "over-and-under") pattern from the confirmed swings.
 *
 * Bullish QM (buy): a swing low (left shoulder), a bounce to a swing high, then
 * a LOWER low (the "head"). Price then breaks back above that swing high
 * (break of structure up) and the buy is taken on the return to the left
 * shoulder low. Stop sits below the head. Bearish QM is the mirror.
 *
 * Returns { bull, bear } where each is null or
 *   { leftShoulder, head, broken, entry, stop, confirmed }
 *   - entry:     the left-shoulder level the trade is taken from
 *   - confirmed: the last close has broken the intervening swing (BOS done)
 */
function quasimodo(candles, wing = 2) {
  const { highs, lows } = swings(candles, wing);
  ...
  if (lows.length >= 2 && highs.length >= 1) {
    const head = lows[lows.length - 1];
    const ls = lows[lows.length - 2];
    if (head.price < ls.price) {
      const between = highs.filter(h => h.idx > ls.idx);
      const broken = (between.length ? between : highs).reduce((a, b) => (b.price > a.price ? b : a));
      out.bull = {
        ...
        entry: ls.price,
        stop: head.price,
        confirmed: close > broken.price,
      };
    }
  }
  if (highs.length >= 2 && lows.length >= 1) {
    const head = highs[highs.length - 1];
    const ls = highs[highs.length - 2];
    if (head.price > ls.price) {
      const between = lows.filter(l => l.idx > ls.idx);
      const broken = (between.length ? between : lows).reduce((a, b) => (b.price < a.price ? b : a));
      out.bear = {
        leftShoulder: ls.price,
        head: head.price,
        broken: broken.price,
...
/**
 * Volume Profile over `candles`: the price range is split into `bins`
 * horizontal levels and each candle's volume is spread evenly across the bins
 * its high–low range covers. When the feed carries no volume (spot metals /
 * FX often report 0), each candle contributes one unit of "time" instead, so
 * the POC still marks the most-accepted price. Returns
 *   { poc, vah, val, source: 'volume' | 'time', lo, hi }
 * or null when there is not enough data.
 */
function volumeProfile(candles, bins = 24) {
  if (!Array.isArray(candles) || candles.length < 10 || bins < 4) return null;
  let lo = Infinity, hi = -Infinity;
  for (const c of candles) { if (c.low < lo) lo = c.low; if (c.high > hi) hi = c.high; }
  if (!(hi > lo)) return null;
  const step = (hi - lo) / bins;
  const vol = new Array(bins).fill(0);
  const haveVol = candles.some(c => c.volume != null && c.volume > 0);
  for (const c of candles) {
    const w = haveVol ? (c.volume > 0 ? c.volume : 0) : 1;
    if (w <= 0) continue;
    let b0 = Math.max(0, Math.min(bins - 1, Math.floor((c.low - lo) / step)));
    let b1 = Math.max(0, Math.min(bins - 1, Math.floor((c.high - lo) / step)));
    const share = w / (b1 - b0 + 1);
  ...
  while (acc < total * 0.7 && (loIdx > 0 || hiIdx < bins - 1)) {
    ...
    if (up >= down) { hiIdx += 1; acc += vol[hiIdx]; } else { loIdx -= 1; acc += vol[loIdx]; }
  }
  const r2 = v => Math.round(v * 100) / 100;
  return {
    poc: r2(lo + step * (pocIdx + 0.5)),
    vah: r2(lo + step * (hiIdx + 1)),
    val: r2(lo + step * loIdx),
    source: haveVol ? 'volume' : 'time',
    lo: r2(lo), hi: r2(hi),
  };
}
