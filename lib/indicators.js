...
/** Bullish rejection candle: a hammer/pin bar (long lower wick) or a bullish
 *  engulfing of the previous candle. `prev` may be null. */
function isBullishRejection(prev, cur) {
  ...
  const pin = lowerWick >= range * 0.5 && body <= range * 0.4 && upperWick <= range * 0.25;
...
function isBearishRejection(prev, cur) {
  ...
  const upperWick = cur.high - Math.max(cur.open, cur.close);
  const lowerWick = Math.min(cur.open, cur.close) - cur.low;
  const pin = upperWick >= range * 0.5 && body <= range * 0.4 && lowerWick <= range * 0.25;
  const engulf = !!prev && prev.close > prev.open && cur.close < cur.open
