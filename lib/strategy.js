...
function institutionalModel(p) {
  ...
  if (!maAligned) {
    esBlockers.push(ms.dir !== dir && ms.dir !== 'MIXED' && ms.dir !== 'NA'
      ...
      : `MA 9/21/50/200 ยังไม่เรียงตัวพอ (${ms.aligned}/4 ตามทาง, ต้อง ≥3)`);
  } else esReasons.push('MA เรียงตัวแข็งแรง (≥3/4)');
  ...
  const rsiConfirms = rsi != null && (buy ? rsi > 55 : rsi < 45);
  if (!rsiConfirms) esBlockers.push(`RSI ${rsi != null ? rsi.toFixed(0) : '?'} ยังไม่ยืนยันฝั่ง${buy ? 'ซื้อ (ต้อง >55)' : 'ขาย (ต้อง <45)'}`);
  const rsiChase = rsi != null && (buy ? rsi > 78 : rsi < 22);
