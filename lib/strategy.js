...
/**
 * Per-entry-TF defaults for the entry-trigger gates, backtest-tuned
 * (5000-bar walk-forward, spread modelled, XAU):
 *   4h    : score>=50/ADX>=18/MA>=3/RSI-confirm → 65% win, +0.62R/trade, PF 2.9
 *   30min : same + require the next structure TF (H1) to agree           → 55%
 *           win, +0.32R/trade, PF 1.8 — real but thin sample (~3.5 mo of data)
 *   1h / 15min / everything else: no backtest config found a positive edge
 *           (every combination tried came back negative or a handful of
 *           trades). Kept selectable for monitoring; entrySignal.reliable
 *           is false so the dashboard never calls these "recommended".
 */
function defaultEntryOpts(interval) {
  if (interval === '4h') return { reliable: true };
  if (interval === '30min') return { requireStructAgree: true, reliable: true };
 ...
 */
function analyze({ assetKey = 'XAU', interval = '1h', entryCandles, structureCandles, trendCandles, entryOpts }) {
  const mergedEntryOpts = { ...defaultEntryOpts(interval), ...(entryOpts || {}) };
  ...
  const institutional = institutionalModel({
    ...
    adx: adxEntry, sessionOK, ema9, ema21, ema50: ema50Entry, ema200, entryOpts: mergedEntryOpts,
  });
...
function institutionalModel(p) {
  const { side, entryCandles, trendCandles, a1, price, trend, structTF, structEntry, bar, qm, srZone, priorDay, rsi, rsiArr, macd, maStack, vprofile, adx, sessionOK, ema9, ema21, ema50, ema200, entryOpts } = p;
  // Tunable entry-trigger gates — defaults are the 5000-bar-backtest-tuned
  // "High-Confidence" settings for XAU/4h. Callers (e.g. per-TF research in
  // backtest.js) can override any of these without touching the model itself.
  const eo = {
    scoreMin: 50, adxMin: 18, maAlignMin: 3, requireRsiConfirm: true,
    rsiBuyMin: 55, rsiSellMax: 45, rsiChaseBuy: 78, rsiChaseSell: 22,
    requireStructAgree: false,   // require the next-higher structure TF to agree too
    reliable: true,               // whether this TF/config combo has backtest support (UI-facing)
    ...(entryOpts || {}),
  };
  ...
  // Backtest-tuned (XAU 4h, 5000-bar walk-forward, spread modelled):
  //   score>=50 + ADX>=18                        → 46 trades, 56% win, PF 2.3
  //   + MA stack aligned>=3 + RSI confirm (below) → 23 trades, 65% win, PF 3.2
  // Fewer signals, much higher hit rate — this is the "high accuracy" mode.
  // All thresholds below are overridable via entryOpts for other TFs.
  if (score < eo.scoreMin) esBlockers.push(`setup context ยังไม่พอ (score < ${eo.scoreMin})`);
  if (sessionOK === false) esBlockers.push('นอกเวลาเทรด (London+NY)');
  const adxOk = !!(adx && adx.adx >= eo.adxMin);
  if (!adxOk || ms.aligned < 1) esBlockers.push(`ตลาดยังไม่มีเทรนด์ชัด (ADX ${adx ? adx.adx.toFixed(0) : '?'} / MA ${ms.aligned}/4)`);
  else esReasons.push(`ADX ${adx.adx.toFixed(0)}`);
  const maAligned = ms.dir === dir && ms.aligned >= eo.maAlignMin;
  if (!maAligned) {
    esBlockers.push(ms.dir !== dir && ms.dir !== 'MIXED' && ms.dir !== 'NA'
      ? `MA เรียงตัวสวนทาง (${ms.aligned}/4 เอียง${ms.dir === 'UP' ? 'ขึ้น' : 'ลง'}, ต้องเอียงตาม${buy ? 'ขึ้น' : 'ลง'})`
      : `MA 9/21/50/200 ยังไม่เรียงตัวพอ (${ms.aligned}/4 ตามทาง, ต้อง ≥${eo.maAlignMin})`);
  } else esReasons.push(`MA เรียงตัวแข็งแรง (≥${eo.maAlignMin}/4)`);
  ...
  if (eo.requireStructAgree && structTF.trend !== dir) esBlockers.push('โครงสร้าง TF ถัดไปยังไม่เห็นด้วยกับทิศทางนี้');
  ...
  // RSI must actually confirm the trade side (not just "not overbought/sold") —
  // this one condition alone was the single biggest lever on win rate in the sweep.
  if (eo.requireRsiConfirm) {
    const rsiConfirms = rsi != null && (buy ? rsi > eo.rsiBuyMin : rsi < eo.rsiSellMax);
    if (!rsiConfirms) esBlockers.push(`RSI ${rsi != null ? rsi.toFixed(0) : '?'} ยังไม่ยืนยันฝั่ง${buy ? `ซื้อ (ต้อง >${eo.rsiBuyMin})` : `ขาย (ต้อง <${eo.rsiSellMax})`}`);
  }
  const rsiChase = rsi != null && (buy ? rsi > eo.rsiChaseBuy : rsi < eo.rsiChaseSell);
  ...
  let entrySignal;
  if (!fire) {
    entrySignal = { fire: false, side, blockers: esBlockers, have: esReasons, reliable: eo.reliable };
  } else {
    const entry = price;
...
// ---------------------------------------------------------------------------
// Mean-reversion trigger — a different bet than the trend-following
// entrySignal above: instead of riding a trend, fade a stretched move back
// toward its average, and ONLY when the market is range-bound (the opposite
// regime filter from the trend model). Self-contained — only needs the entry
// TF's own candles, no higher-TF trend/structure required.
//
//   fires when: NOT trending (ADX <= adxMax) · in session · price at/beyond a
//   Bollinger band · RSI at the matching extreme · a rejection candle closes
//   back inside the band
//   entry = close · SL = beyond the recent extreme · TP1 = the BB basis
//   (the mean) · TP2 = the opposite band (a full range swing, rare)
// ---------------------------------------------------------------------------
function meanReversion(entryCandles, opts = {}) {
  const eo = {
    adxMax: 22, rsiOversold: 32, rsiOverbought: 68, bbMult: 2.0, bbPeriod: 20,
    touchAtr: 0.15, slBufferAtr: 0.3, swingLookback: 8, sessionCfg: null,
    ...opts,
  };
  const n = entryCandles.length;
  if (n < Math.max(60, eo.bbPeriod + 10)) return { fire: false, side: null, blockers: ['ข้อมูลไม่พอสำหรับ mean reversion'], have: [] };

  const closes = entryCandles.map(c => c.close);
  const price = closes[n - 1];
  const atr = I.last(I.atrSeries(entryCandles, 14));
  const a = isFinite(atr) && atr > 0 ? atr : price * 0.002;
  const bb = I.bollinger(closes, eo.bbPeriod, eo.bbMult);
  const rsiArr = I.rsiSeries(closes, 14);
  const rsi = I.last(rsiArr);
  const adx = I.adxLatest(entryCandles, 14);
  const cur = entryCandles[n - 1], prev = entryCandles[n - 2] || null;
  const sessionOK = eo.sessionCfg ? inSession(cur.time, eo.sessionCfg) : true;

  const blockers = [], have = [];
  if (!bb || rsi == null) { blockers.push('ข้อมูลไม่พอ (Bollinger/RSI)'); return { fire: false, side: null, blockers, have }; }
  if (sessionOK === false) blockers.push('นอกเวลาเทรด (London+NY)');
  const ranging = !!(adx && adx.adx <= eo.adxMax);
  if (!ranging) blockers.push(`ตลาดมีเทรนด์แรงเกิน (ADX ${adx ? adx.adx.toFixed(0) : '?'} > ${eo.adxMax}) — เสี่ยงสวนเทรนด์`);
  else have.push(`ADX ${adx.adx.toFixed(0)} ≤ ${eo.adxMax} (ไซด์เวย์)`);

  const touchLower = price <= bb.lower + a * eo.touchAtr;
  const touchUpper = price >= bb.upper - a * eo.touchAtr;
  let side = null;
  if (touchLower && rsi <= eo.rsiOversold) side = 'BUY';
  else if (touchUpper && rsi >= eo.rsiOverbought) side = 'SELL';
  if (!side) {
    blockers.push('ราคายังไม่แตะขอบ Bollinger พร้อม RSI สุดขั้ว');
    return { fire: false, side: null, blockers, have, bb, rsi, adx: adx ? adx.adx : null };
  }
  const buy = side === 'BUY';
  have.push(`ราคาแตะ Band ${buy ? 'ล่าง' : 'บน'} + RSI ${rsi.toFixed(0)} ${buy ? '≤' : '≥'} ${buy ? eo.rsiOversold : eo.rsiOverbought}`);

  const rejBar = buy
    ? (I.isBullishRejection(prev, cur) ? cur : null)
    : (I.isBearishRejection(prev, cur) ? cur : null);
  if (!rejBar) blockers.push(`ยังไม่มีแท่ง ${buy ? 'bullish' : 'bearish'} rejection ยืนยันที่ขอบ Band`);
  else have.push('แท่ง rejection ที่ขอบ Band ยืนยัน');

  const fire = blockers.length === 0 && !!rejBar;
  const r2 = v => (v == null ? null : Math.round(v * 100) / 100);
  if (!fire) return { fire: false, side, blockers, have, bb, rsi, adx: adx ? adx.adx : null };

  const recent = entryCandles.slice(-eo.swingLookback - 1, -1);
  const extreme = buy
    ? Math.min(rejBar.low, ...recent.map(c => c.low))
    : Math.max(rejBar.high, ...recent.map(c => c.high));
  const entry = price;
  const sl = buy ? extreme - a * eo.slBufferAtr : extreme + a * eo.slBufferAtr;
  const risk = Math.abs(entry - sl);
  const tp1 = bb.basis;                                   // the mean — primary target
  const tp2 = buy ? bb.upper : bb.lower;                   // opposite band — full swing, rare
  return {
    fire: true, side, have, blockers: [],
    entry: r2(entry), sl: r2(sl), tp1: r2(tp1), tp2: r2(tp2),
    risk: r2(risk), rr1: risk > 0 ? Math.round((Math.abs(tp1 - entry) / risk) * 100) / 100 : null,
    bb, rsi, adx: adx ? adx.adx : null,
    trigger: `ราคาแตะ Band ${buy ? 'ล่าง' : 'บน'} + RSI สุดขั้ว + rejection ในตลาดไซด์เวย์ (ADX≤${eo.adxMax}) — เป้าคือกลับไปที่เส้นกลาง`,
  };
}

module.exports = {
  analyze, emaTrend, tfStack, higherInterval, trendInterval,
  assetConfig, ASSET_CONFIG, SIGNAL_STACK, HTF_MAP, meanReversion,
};
