...
function institutionalModel(p) {
  ...
  for (const lvl of sweepTargets) {
    ...
    if (pierced && sweep < 10) { sweep = 10; sweepLevel = lvl; sweepNote = `แตะ${w.under} ${lvl.toFixed(2)} — รอปิดกลับ${w.over}ระดับเพื่อยืนยัน`; }
  }
  ...
  if (qm) {
    ...
    dz = 12; dzNote = `ราคาอยู่ใน ${w.zone} Zone ${srZone.lo.toFixed(2)}–${srZone.hi.toFixed(2)} (ทดสอบ ${srZone.touches} ครั้ง)`;
  } else if (srZone) {
    dz = 5; dzNote = `มี ${w.zone} Zone ${srZone.lo.toFixed(2)}–${srZone.hi.toFixed(2)} แต่ราคายังไม่ถึง`;
  }
  ...
  if (strong) { disp += 9; dispBits.push(`แท่ง${w.bar} Displacement (ช่วง > 1.3×ATR)`); }
    ...
    if (mid) { disp += 5; dispBits.push(`แท่ง${w.mss}แรงพอควร`); }
  }
  ...
  else if (bar.bos === dir && bar.level != null && Math.abs(price - bar.level) <= atr * 0.6) { pb = 5; pbNote = `ราคากำลังกลับมาที่ ${bar.level.toFixed(2)} — รอสัญญาณยืน`; }
  ...
  const tier = score >= 80 ? 'STRONG_ZONE' : score >= 55 ? 'FORMING' : 'WAIT';
