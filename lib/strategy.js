...
function analyze({ assetKey = 'XAU', interval = '1h', entryCandles, structureCandles, trendCandles }) {
  ...
  const institutional = isBuy
    ? institutionalBuy({
        entryCandles, trendCandles, a1, price, trend, structTF, structEntry, bar,
        qm: qm.bull, srZone, priorDay: pdl, rsi, rsiArr, ema50Entry,
      })
    : null;
...
/**
 * Institutional Accumulation (BUY-only) score, /100 — the "smart money buys the
 * dip" playbook: HTF uptrend → liquidity below → sweep + reclaim of a prior low
 * → QM / demand zone → market-structure shift up → displacement → the retest.
 * >=80 = STRONG BUY ZONE. Returns null-equivalent state for non-uptrends via
 * the caller (only invoked when direction === 'BUY').
 */
function institutionalBuy(p) {
  const { entryCandles, trendCandles, a1, price, trend, structTF, structEntry, bar, qm, srZone, priorDay, rsi, rsiArr, ema50Entry } = p;
  ...
  // 0) HTF Trend (+15)
  if (trend.dir === 'UP') add('htf', 'HTF Trend', 15, 15, 'ราคาเหนือ EMA · แนวโน้มหลักขาขึ้น');
  else if (price != null && ema50Entry != null && price > ema50Entry) add('htf', 'HTF Trend', 15, 7, 'ราคายังเหนือ EMA แต่ความชันไม่ชัด');
  else add('htf', 'HTF Trend', 15, 0, 'แนวโน้มหลักยังไม่เป็นขาขึ้น');

  // 1) Liquidity Sweep (+20): a swept prior low that price reclaimed
  const lows = I.swings(entryCandles, 2).lows;
  const refLow = lows.length ? lows[lows.length - 1].price : (structEntry.lastLow ? structEntry.lastLow.price : null);
  const pdLow = priorDay ? priorDay.low : null;
  const sweepTargets = [refLow, pdLow].filter(v => v != null && v < price);
  let sweep = 0, sweepNote = 'ยังไม่เห็นการกวาดสภาพคล่องใต้ Low เดิม', sweepLevel = null;
  const swWindow = entryCandles.slice(-10);
  for (const lvl of sweepTargets) {
    const dipped = swWindow.some(c => c.low < lvl - atr * 0.1);
    if (dipped && price > lvl) { sweep = 20; sweepLevel = lvl; sweepNote = `กวาด Low ${lvl.toFixed(2)} แล้วดีดกลับเหนือระดับ (Sweep + Reclaim)`; break; }
    if (dipped && sweep < 10) { sweep = 10; sweepLevel = lvl; sweepNote = `แตะใต้ Low ${lvl.toFixed(2)} — รอปิดกลับเหนือระดับเพื่อยืนยัน`; }
  }
  add('sweep', 'Liquidity Sweep', 20, sweep, sweepNote);

  // 2) QM / Demand Zone (+20)
  let dz = 0, dzNote = 'ยังไม่พบ QM / Demand Zone';
  if (qm) {
    dz = qm.confirmed ? 20 : 10;
    dzNote = qm.confirmed
      ? `QM ฝั่งซื้อครบ — ไหล่ ${qm.leftShoulder.toFixed(2)} / หัว ${qm.head.toFixed(2)} · ทะลุ ${qm.broken.toFixed(2)} แล้ว`
      : `QM ฝั่งซื้อกำลังก่อตัว — ยังไม่ทะลุ ${qm.broken.toFixed(2)}`;
  } else if (srZone && price >= srZone.lo - atr * 0.35 && price <= srZone.hi + atr * 0.35) {
    dz = 12; dzNote = `ราคาอยู่ใน Demand Zone ${srZone.lo.toFixed(2)}–${srZone.hi.toFixed(2)} (ทดสอบ ${srZone.touches} ครั้ง)`;
  } else if (srZone) {
    dz = 5; dzNote = `มี Demand Zone ${srZone.lo.toFixed(2)}–${srZone.hi.toFixed(2)} แต่ราคายังไม่ถึง`;
  }
  add('demand', 'QM / Demand Zone', 20, dz, dzNote);

  // 3) Market Structure Shift (+20)
  let mss = 0; const mssBits = [];
  if (bar.bos === 'UP') { mss += 12; mssBits.push(`BOS ขาขึ้นที่ ${bar.level != null ? bar.level.toFixed(2) : '-'}`); }
  if (structTF.trend === 'UP' || structTF.bos === 'UP' || structTF.choch === 'UP') { mss += 8; mssBits.push('โครงสร้าง TF ถัดไปเป็นขาขึ้น (HH/HL)'); }
  add('mss', 'Market Structure Shift', 20, mss, mssBits.length ? mssBits.join(' · ') : 'โครงสร้างยังไม่เปลี่ยนเป็นขาขึ้น');
  ...
  const strong = last3.find(c => {
    ...
    return range > 0 && c.close > c.open && (c.close - c.open) >= range * 0.55 && range >= atr * 1.3;
  });
  if (strong) { disp += 9; dispBits.push('แท่งเขียว Displacement (ช่วง > 1.3×ATR)'); }
  else {
    const mid = last3.find(c => (c.high - c.low) >= atr && c.close > c.open);
    if (mid) { disp += 5; dispBits.push('แท่งขาขึ้นแรงพอควร'); }
  }
  ...
  if (bar.bos === 'UP' && bar.retest) { pb = 10; pbNote = `ราคาย่อกลับมาทดสอบ ${bar.level.toFixed(2)} แล้วยืนได้ (Retest ผ่าน)`; }
  else if (bar.bos === 'UP' && bar.level != null && Math.abs(price - bar.level) <= atr * 0.6) { pb = 5; pbNote = `ราคากำลังย่อกลับมาที่ ${bar.level.toFixed(2)} — รอสัญญาณยืน`; }
  ...
  const rsiRising = rClean.length >= 3 ? rClean[rClean.length - 1] > rClean[rClean.length - 3] : null;
  const rsiOk = rsi != null && rsi > 50 && rsiRising !== false;
  const tier = score >= 80 ? 'STRONG_BUY_ZONE' : score >= 55 ? 'FORMING' : 'WAIT';

  // Liquidity targets above — where price is likely drawn
  const swHigh = structEntry.lastHigh ? structEntry.lastHigh.price : null;
  const legLow = sweepLevel != null ? sweepLevel : (structEntry.lastLow ? structEntry.lastLow.price : null);
  const trendHigh = trendCandles && trendCandles.length ? I.marketStructure(trendCandles, 2).lastHigh : null;
  const targets = [];
  const pushT = (name, v) => { if (v != null && isFinite(v) && v > price) targets.push({ name, price: Math.round(v * 100) / 100 }); };
  pushT('Previous Day High', priorDay ? priorDay.high : null);
  pushT('Swing / Equal High', swHigh);
  if (legLow != null && swHigh != null && swHigh > legLow) {
    const range = swHigh - legLow;
    pushT('Fib Ext 1.272', swHigh + range * 0.272);
    pushT('Fib Ext 1.618', swHigh + range * 0.618);
  }
  pushT('Weekly / Daily Supply', trendHigh ? trendHigh.price : null);
  targets.sort((a, b) => a.price - b.price);

  return {
    state: 'ACCUMULATION',
    score, maxScore: 100, tier,
    ready: tier === 'STRONG_BUY_ZONE',
    components: comps,
    sweepLevel,
    ...
    checklist: [
      { name: 'ราคาทำ Sweep Low กวาดสภาพคล่อง', got: sweep >= 20 },
      { name: 'อยู่ใน Demand Zone / QM', got: dz >= 12 },
      { name: 'เกิด Market Structure Shift (MSS/BOS)', got: mss >= 12 },
      { name: 'มีแท่งเขียว Displacement + Volume สูง', got: disp >= 9 },
      { name: 'Retest โซนแล้วไม่หลุด', got: pb >= 10 },
      { name: 'RSI > 50 และดีดขึ้น / ไม่มี Divergence ขาลง', got: !!rsiOk },
    ],
  };
}
...
function buildResult(x) {
  ...
  return {
    ...
    institutional: x.institutional || {
      state: x.direction === 'SELL' ? 'DISTRIBUTION' : 'NO_TREND',
      score: 0, maxScore: 100, tier: 'WAIT', ready: false,
      components: [], targets: [], checklist: [],
      rsi: x.rsi != null ? Math.round(x.rsi * 10) / 10 : null, rsiRising: null, rsiOk: false,
      note: x.direction === 'SELL'
        ? 'เทรนด์ H4 เป็นขาลง — ธนาคารยังไม่สะสม (โหมดเข้าซื้อไม่ทำงาน) รอโครงสร้างกลับเป็นขาขึ้น'
        : 'เทรนด์ H4 ยังไม่ชัดเจน — ยังไม่มีโซนสะสม',
    },
    indicators: {
      ema50: x.ema50Entry,
