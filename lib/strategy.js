...
function analyze({ assetKey = 'XAU', interval = '1h', entryCandles, structureCandles, trendCandles }) {
  ...
  if (!direction) {
    ...
    return buildResult({ assetKey, interval, stack, price, atr, ema9, ema21, ema50Entry, ema200, rsi, macd, maStack, trend, trendDirWord, structTF, structWord, structEntry, bar, direction: null, score: 0, checklist, cfg, entryTime: entryCandles[n - 1].time });
  }
  ...
  const institutional = institutionalModel({
    ...
    qm: isBuy ? qm.bull : qm.bear, srZone, priorDay: pdl, rsi, rsiArr, macd, maStack,
    ema9, ema21, ema50: ema50Entry, ema200,
  });

  return buildResult({ assetKey, interval, stack, price, atr, ema9, ema21, ema50Entry, ema200, rsi, macd, maStack, trend, trendDirWord, structTF, structWord, structEntry, bar, direction, score, checklist, srLevel, srZone, isHtfLevel, priorDay: pdl, atSR, bos, retest, rej, qm: qmSide, qmMatch, institutional, cfg, entryTime: entryCandles[n - 1].time });
}
...
function institutionalModel(p) {
  const { side, entryCandles, trendCandles, a1, price, trend, structTF, structEntry, bar, qm, srZone, priorDay, rsi, rsiArr, macd, maStack, ema9, ema21, ema50, ema200 } = p;
  ...
  // 0) Trend & MA Stack (+15): higher-TF EMA trend + entry-TF EMA 9/21/50/200 alignment
  let htf = 0; const htfBits = [];
  if (trend.dir === dir) { htf += 6; htfBits.push(`เทรนด์หลัก ${w.trendW}`); }
  else if (ema200 != null && cmp(price, ema200)) { htf += 3; htfBits.push(`ราคา${w.over} EMA200`); }
  if (ms.dir === dir && ms.aligned === 4) { htf += 9; htfBits.push(ms.label); }
  else if (ms.dir === dir && ms.aligned === 3) { htf += 6; htfBits.push(ms.label); }
  else if (ema50 != null && cmp(price, ema50)) { htf += 3; htfBits.push(`ราคา${w.over} EMA50 (MA ยังไม่เรียงเต็ม)`); }
  ...
  if (strong) { disp += 5; dispBits.push(`แท่ง${w.bar} Displacement > 1.3×ATR`); }
  else {
  ...
  // MACD histogram in the trade direction (+4)
  if (macd && isFinite(macd.histogram)) {
    const favor = buy ? macd.histogram > 0 : macd.histogram < 0;
    const expanding = buy ? macd.histogram > macd.histogramPrev : macd.histogram < macd.histogramPrev;
    if ((buy && macd.crossUp) || (!buy && macd.crossDown)) { disp += 4; dispBits.push(`MACD ตัด${buy ? 'ขึ้น' : 'ลง'} (cross)`); }
    else if (favor && expanding) { disp += 4; dispBits.push(`MACD histogram ${buy ? 'บวกและกว้างขึ้น' : 'ลบและกว้างขึ้น'}`); }
    else if (favor) { disp += 2; dispBits.push(`MACD ${buy ? 'ยังบวก' : 'ยังลบ'} แต่แผ่ว`); }
  }
  // RSI momentum on the correct side (+3)
  if (rsi != null) {
    if (buy ? (rsi > 50 && rsiRising !== false) : (rsi < 50 && rsiRising !== true)) { disp += 3; dispBits.push(`RSI ${rsi.toFixed(0)} ${buy ? '> 50 ดีดขึ้น' : '< 50 อ่อนลง'}`); }
    else if (buy ? rsi > 45 : rsi < 55) { disp += 1; dispBits.push(`RSI ${rsi.toFixed(0)} ใกล้พลิก`); }
  }
