...
function institutionalModel(p) {
  const { side, entryCandles, trendCandles, a1, price, trend, structTF, structEntry, bar, qm, srZone, priorDay, rsi, rsiArr, macd, maStack, vprofile, ema9, ema21, ema50, ema200 } = p;
  ...
  return {
    ...
    sweepLevel,
    entryPlan,
    // Exit plan tuned for a >=70% win rate: bank most at the 0.5R first target,
    // move the stop to breakeven, let a runner ride to 2R / 4R.
    plan: { tp1R: 0.5, tp2R: 2, tp3R: 4, beAfter: 'TP1', primaryTP: 'TP1' },
    maStack: ms,
