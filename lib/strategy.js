...
function analyze({ assetKey = 'XAU', interval = '1h', candles, htfCandles }) {
  ...
  //    within the trend rather than chasing an extended move.
  let pullback = null;
  if (emaFast != null && emaSlow != null && atr != null && atr > 0) {
    ...
    if (inBand && regime.direction === 'UP') { pullback = 'BUY'; vote(1, 1, 'ราคาย่อกลับเข้าโซน EMA (ซื้อตามเทรนด์ ไม่ไล่ราคา)'); }
    else if (inBand && regime.direction === 'DOWN') { pullback = 'SELL'; vote(-1, 1, 'ราคาเด้งกลับเข้าโซน EMA (ขายตามเทรนด์ ไม่ไล่ราคา)'); }
  }
  ...
  const MAX_WEIGHT = 12; // regime 3 + cascade 3 + structure 2 + macd 1 + rsi 1 + adx 1 + pullback 1
