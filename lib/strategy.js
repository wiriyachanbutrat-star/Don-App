...
function institutionalModel(p) {
  ...
  if (score < 55) esBlockers.push('setup context ยังไม่พอ (score < 55)');
  if (sessionOK === false) esBlockers.push('นอกเวลาเทรด (London+NY)');
  const adxOk = !!(adx && adx.adx >= 20);
  const maAligned = ms.dir === dir && ms.aligned >= 3;
  if (!adxOk || ms.aligned < 2) esBlockers.push(`ตลาดยังไม่มีเทรนด์ชัด (ADX ${adx ? adx.adx.toFixed(0) : '?'} / MA ${ms.aligned}/4)`);
