...
const { analyze, meanReversion, assetConfig } = require('./strategy');
...
function runTriggerBacktest({
  assetKey = 'XAU', interval = '1h', candles, structureCandles, trendCandles,
  spread = 0.30, maxHold = MAX_HOLD_BARS, cooldownBars = 4, lossCooldownBars = 6,
  partialAtTP1 = 0.5, trailAtr = 0,   // trailAtr 0 = fixed TP2 exit; >0 = chandelier trail on the runner
  extraFilter = null,   // optional (institutional, entrySignal) => bool — additional gate for research/tuning
  entryOpts = null,      // optional overrides for institutionalModel's entry-trigger thresholds (per-TF tuning)
}) {
  const n = candles.length;
  if (n < WINDOW + 50) return { error: `ข้อมูลย้อนหลังน้อยเกินไป (${n} แท่ง)` };
  const sortByTime = (a, b) => (a.time < b.time ? -1 : 1);
  ...
  const trades = [];
  let i = WINDOW, lastExitIdx = -9999, lastLoss = false;
  while (i < n - 1) {
    const bar = candles[i];
    while (sIdx < structSorted.length && structSorted[sIdx].time <= bar.time) sIdx++;
    while (tIdx < trendSorted.length && trendSorted[tIdx].time <= bar.time) tIdx++;
    const cd = lastLoss ? lossCooldownBars : cooldownBars;
    if (sIdx < 60 || tIdx < 60 || i - lastExitIdx < cd) { i++; continue; }
    ...
    if (extraFilter && !extraFilter(res.institutional, es, res)) { i++; continue; }
    ...
    // Bank partialAtTP1 at TP1, move the runner's stop to breakeven, then either
    // exit the rest at TP2 (trailAtr 0) or chandelier-trail it by trailAtr*ATR.
    ...
    for (let j = i + 1; j <= lastScan; j++) {
      ...
      if (!tp1Hit) {
        ...
        if (t1Hit) { tp1Hit = true; rTotal += partialAtTP1 * (Math.abs(tp1 - entry) - spread) / risk; stop = entry; best = entry; }
      } else if (trailAtr > 0) {
        best = isBuy ? Math.max(best, c.high) : Math.min(best, c.low);
        const trail = isBuy ? best - trailAtr * atrHere : best + trailAtr * atrHere;
        stop = isBuy ? Math.max(stop, trail) : Math.min(stop, trail);
        if (isBuy ? c.low <= stop : c.high >= stop) {
          rTotal += (1 - partialAtTP1) * ((isBuy ? (stop - entry) : (entry - stop)) - spread) / risk;
          exitIdx = j; break;
        }
      } else {
...
// ---------------------------------------------------------------------------
// Walk-forward backtest of the MEAN-REVERSION trigger (strategy.meanReversion):
// fade a stretched move back to the Bollinger basis while the market is
// range-bound. Single-TF, no higher-TF trend/structure needed. Banks
// `partialAtTP1` at the mean (TP1), lets the rest run to the opposite band
// (TP2) with the stop at breakeven.
// ---------------------------------------------------------------------------
function runMeanReversionBacktest({
  assetKey = 'XAU', interval = '15min', candles,
  spread = 0.30, maxHold = 24, cooldownBars = 4, partialAtTP1 = 0.6,
  meanReversionOpts = null,
}) {
  const n = candles.length;
  const WARMUP = 60;
  if (n < WARMUP + 50) return { error: `ข้อมูลย้อนหลังน้อยเกินไป (${n} แท่ง)` };
  const cfg = assetConfig(assetKey);
  const mrOpts = { sessionCfg: cfg.session, ...(meanReversionOpts || {}) };

  const trades = [];
  let i = WARMUP, lastExitIdx = -9999;
  while (i < n - 1) {
    if (i - lastExitIdx < cooldownBars) { i++; continue; }
    const bar = candles[i];
    const window = candles.slice(Math.max(0, i - 200), i + 1);
    let mr;
    try { mr = meanReversion(window, mrOpts); } catch (e) { i++; continue; }
    if (!mr || !mr.fire) { i++; continue; }

    const isBuy = mr.side === 'BUY';
    const entry = bar.close + (isBuy ? spread / 2 : -spread / 2);
    const sl = mr.sl;
    const risk = Math.abs(entry - sl);
    if (!(risk > 0)) { i++; continue; }
    const tp1 = mr.tp1, tp2 = mr.tp2;
    ...
    for (let j = i + 1; j <= lastScan; j++) {
      ...
      if (!tp1Hit) {
        ...
        const t1Hit = isBuy ? c.high >= tp1 : c.low <= tp1;
        if (slHit) { outcome = 'loss'; rTotal = -(risk + spread) / risk; exitIdx = j; break; }
        if (t1Hit) { tp1Hit = true; rTotal += partialAtTP1 * (Math.abs(tp1 - entry) - spread) / risk; stop = entry; }
      } else {
        const beHit = isBuy ? c.low <= stop : c.high >= stop;
        const t2Hit = isBuy ? c.high >= tp2 : c.low <= tp2;
        if (t2Hit) { rTotal += (1 - partialAtTP1) * (Math.abs(tp2 - entry) - spread) / risk; exitIdx = j; break; }
    ...
    trades.push({ time: bar.time, direction: mr.side, outcome, rMultiple: rTotal, rr1: mr.rr1, barsHeld: exitIdx - i });
    lastExitIdx = exitIdx;
    i = exitIdx + 1;
  }
  ...
  return {
    mode: 'mean-reversion', interval, params: { spread, partialAtTP1, maxHold, cooldownBars, ...mrOpts },
    period: { from: candles[WARMUP] ? candles[WARMUP].time : null, to: candles[n - 1] ? candles[n - 1].time : null, bars: n },
    totalTrades: trades.length, closedTrades: closed.length, wins, losses: closed.length - wins,
    scratches: trades.filter(t => t.outcome.startsWith('scratch')).length,
    winRate: closed.length ? wins / closed.length : null,
    expectancyR: trades.length ? Math.round((sumR / trades.length) * 1000) / 1000 : null,
...
module.exports = { runBacktest, runInstitutionalBacktest, runTriggerBacktest, runMeanReversionBacktest, WINDOW, MAX_HOLD_BARS };
