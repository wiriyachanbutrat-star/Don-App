...
จุดเข้าแม่นที่ระบบเสนอ: ${ep.price != null ? fmt(ep.price) + ' (อิง ' + ep.basis + ')' : '—'}
Entry trigger: ${I.entrySignal && I.entrySignal.fire
    ? `ยิงแล้ว — Entry ${fmt(I.entrySignal.entry)} / SL ${fmt(I.entrySignal.sl)} / TP1 ${fmt(I.entrySignal.tp1)} (RR≈1:${I.entrySignal.rr1}) / TP2 ${fmt(I.entrySignal.tp2)} / TP3 ${fmt(I.entrySignal.tp3)}`
    : `ยังไม่ยิง — ขาด: ${(I.entrySignal && I.entrySignal.blockers || []).join(', ') || '—'}`}
ADX(entry): ${I.adx ? fmt(I.adx.adx, 1) : '—'} · ในเวลาเทรด: ${I.sessionOK ? 'ใช่' : 'ไม่'}
